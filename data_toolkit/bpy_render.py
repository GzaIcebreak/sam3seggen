...
class BpyRenderer:
    ...
    def normalize_scene(self, ref_bbox=None):
        """Fit the loaded scene into a unit cube centred on the origin.

        By default the scale/offset are derived from the loaded object's own bounding
        box, which is right for viewing a single complete model but misleading for a
        part split out of a larger assembly: each part gets re-centred and rescaled to
        fill the frame on its own, so it ends up looking "rotated" relative to the whole
        body even though its geometry never moved. Pass `ref_bbox` (the bbox of the full
        assembly, in the same object-space) to normalize every part with the same
        scale/offset instead, so split parts keep their original relative pose.
        """
        ...
        ref_min, ref_max = (mathutils.Vector(ref_bbox[0]), mathutils.Vector(ref_bbox[1])) if ref_bbox else (bbox_min, bbox_max)
        loc_before = scene.matrix_world.translation.copy()
        scale = 1 / max(ref_max - ref_min)
        scene.scale = scene.scale * scale
        bpy.context.view_layer.update()
        # Scaling ``scene.scale`` leaves its location untouched, so a world point P moves
        # to loc + scale*(P - loc). Re-centering the reference bbox (not necessarily this
        # object's own bbox) under that same map keeps a part's pose relative to the
        # full assembly it was split from, instead of re-centering on itself.
        ref_center_after = loc_before + scale * ((ref_min + ref_max) / 2 - loc_before)
        offset = -ref_center_after
        scene.matrix_world.translation += offset
        bpy.ops.object.select_all(action="DESELECT")
        return scale, offset

    def set_camera_from_matrix(self, cam, transform_matrix, azimuth_deg=0):
        """Place the camera from a transforms.json entry, optionally orbited around Z.

        The TRACK_TO constraint set up in init_camera always re-aims the camera at the
        origin, so rotating the whole camera-to-world matrix about the world Z (up) axis
        simply orbits it around the model at the same height/radius -- an easy way to get
        extra viewing angles (e.g. a front view) from a single calibrated transform.
        """
        matrix = mathutils.Matrix(transform_matrix)
        if azimuth_deg:
            orbit = mathutils.Matrix.Rotation(math.radians(azimuth_deg), 4, "Z")
            matrix = orbit @ matrix
    ...
    def render_from_transforms(self, file_path, transforms_json_path, output_path, ref_glb=None, azimuths=None):
        """Render one view, or several orbit views if `azimuths` has more than one entry.

        With multiple azimuths, `output_path` is treated as a template: each view is saved
        next to it with the azimuth degrees appended before the extension (e.g. "view.png"
        -> "view_0.png", "view_90.png", "view_180.png"). Returns the list of paths written.
        """
        ...
        ref_bbox = self.compute_bbox(ref_glb) if ref_glb else None
        ...
        scale, offset = self.normalize_scene(ref_bbox=ref_bbox)
        ...
        azimuths = list(azimuths) if azimuths else [0]
        written = []
        root, ext = os.path.splitext(output_path)
        for azimuth_deg in azimuths:
            self.set_camera_from_matrix(cam, transform_matrix, azimuth_deg=azimuth_deg)
            path = output_path if len(azimuths) == 1 else f"{root}_{azimuth_deg:g}{ext}"
