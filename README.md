...
6. **补全**（由 `--complete` 开关控制）：我们的部件在切口处是敞开的，X-Part 拿整体模型加
   一个包围盒提示把每个部件重新生成为封闭实体（`xpart_complete.py`）。先看 `boxes` 模式：
   包围盒是有损提示，一个盒子和邻居重叠得多的部件，生成出来会把盒子填满。

第 3、6 步要花 GPU 时间，其余在渲染图缓存后都是秒级。

采样数是拆分质量的主要旋钮，而且不能用「把每个样本读细」替代：Mickey 上 5 次采样只有 13 个
原子、最大的占 48%，耳朵从未与头分开；把 `--color_tol` 从 20 降到 3 让每样本标签数翻了二十
倍，原子数仍然是 13（多出来的全是碎斑）。改成 9 次采样才得到 40 个原子、最大 19%，`ear` 和
`arm` 随即出现。加大 `--azimuth_jitter` 不是替代品且可能倒退：60 度时有几次采样只出了 2~6 个
标签，这些近乎空白的分区会让 meet 沿着不存在的边界碎裂。
